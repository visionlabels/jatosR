...
Takes raw results stored in `data_raw` and adds new column `data_json`
containing data in lists created by `jsonlite` package. The package
`jsonlite` must be installed for this function to work. It is expected
that the parameter `d` is the dataset obtained with `process_results`,
but the dataset can be simpler, because only the column `data_raw` is
used.
...
Tibble with new list column `data_json`, or the input list with its
`data` tibble replaced by the same (see Details)

## Details

Function `process_results` returns a list (let's assume called
`results`), `raw_json` is intended to be used in tibble pipeline, i.e.
on the `result$data` tibble. To simplify the process, the function
