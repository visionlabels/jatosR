...
#' }
raw_json <- function(d) {
  stopifnot("Package jsonlite must be installed" = requireNamespace("jsonlite", quietly = TRUE))
...
raw_jspsych <- function(d) {
  stopifnot("Package jsonlite must be installed" = requireNamespace("jsonlite", quietly = TRUE))
  stopifnot("Package jspsychread must be installed" = requireNamespace("jspsychread", quietly = TRUE))
  if (tibble::is_tibble(d)) {
    result <-
      d |>
      dplyr::mutate(
